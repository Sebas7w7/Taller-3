...
class FuncionesMatricesTest extends AnyFunSuite {
  ...
  test("Producto punto de dos vectores normales") {
    assert(taller3.prodPunto(Vector(1, 2, 3), Vector(4, 5, 6)) == 32)
  }

  test("Producto punto de vectores con ceros") {
    assert(taller3.prodPunto(Vector(0, 0, 0), Vector(4, 5, 6)) == 0)
  }

  test("Producto punto de vectores unitarios") {
    assert(taller3.prodPunto(Vector(1, 1, 1), Vector(1, 1, 1)) == 3)
  }
  ...
  test("Producto punto de vectores con un solo elemento") {
    assert(taller3.prodPunto(Vector(5), Vector(7)) == 35)
  }

  // Tests para la función transpuesta
  test("Transpuesta de una matriz cuadrada") {
    assert(taller3.transpuesta(Vector(
      Vector(1, 2, 3),
      Vector(4, 5, 6),
      Vector(7, 8, 9)
    )) == Vector(
  ...
  test("Transpuesta de una matriz de una sola fila") {
    assert(taller3.transpuesta(Vector(Vector(1, 2, 3))) == Vector(Vector(1), Vector(2), Vector(3)))
  }

  test("Transpuesta de una matriz de una sola columna") {
    assert(taller3.transpuesta(Vector(Vector(1), Vector(2), Vector(3))) == Vector(Vector(1, 2, 3)))
  }

  test("Transpuesta de una matriz vacía") {
    assert(taller3.transpuesta(Vector()) == Vector())
  }

  // Tests para la función multMatriz
  test("Multiplicación de matrices 2x2") {
    assert(taller3.multMatriz(
